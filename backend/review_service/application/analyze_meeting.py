...
def analyze_meeting_use_case(
    ...
    if len(utterances) == 1:
        u = utterances[0]
        speaker_stats = {u.speaker: {"words": len(u.text.split()), "utterances": 1}}

        # Проверяем офтоп по эвристике
        off_topic_segments = []
        main_topic_texts = []
        ...
        return MeetingReport(
            inferred_topic=main_topic_texts[0] if main_topic_texts else "Не удалось определить тему",
